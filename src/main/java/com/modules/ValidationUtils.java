...
public class ValidationUtils {
	...
	public static final String PESEL_REGEX = "^\\d{11}$";
	public static final String PHONE_NUMBER_REGEX = "^\\d{3}-\\d{3}-\\d{3}$";
	...
	public static void validateIdNumber(Errors errors, String fieldName, String fieldValue, String errorMessage) {
		Matcher matcher = Pattern.compile(PESEL_REGEX).matcher(fieldValue.trim());
		if (!matcher.matches()) {
			errors.rejectValue(fieldName, errorMessage, errorMessage);
		}
	}
	
	public static void validatePhoneNumber(Errors errors, String fieldName, String fieldValue, String errorMessage) {
		Matcher matcher = Pattern.compile(PHONE_NUMBER_REGEX).matcher(fieldValue.trim());
		if (!matcher.matches()) {
			errors.rejectValue(fieldName, errorMessage, errorMessage);
		}
	}
